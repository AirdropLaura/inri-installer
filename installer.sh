...
show_detected_specs() {
    ...
    echo -e "${GREEN}Total RAM:${NC} $(free -h | awk '/^Mem:/{print $2}')"    echo -e "${GREEN}Free RAM:${NC} $(free -h | awk '/^Mem:/{print $4}')"
    echo ""
    echo -e "${YELLOW}========================================${NC}"
    ...
    echo ""
    local EXPECTED_CPU=$(echo "$MINER_THREADS * 100" | bc)
    echo -e "${YELLOW}Expected CPU Usage:${NC} ~${EXPECTED_CPU}%"
    echo -e "${BLUE}========================================${NC}"
    echo ""
}
quick_setup() {
...
EOF
    ...
    echo -e "${GREEN}Mining Threads:${NC} $MINER_THREADS / $(nproc) cores"    echo -e "${GREEN}Cache:${NC} ${CACHE_SIZE}MB"
...
status_check() {
    ...
    if [ -S "$DATADIR/geth.ipc" ]; then
        ...
        echo -e "${YELLOW}IPC not available - service starting...${NC}"
    fi
    echo ""
